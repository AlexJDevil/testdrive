...
const (
	StackName    = "TestDriveStack"
	FunctionName = "testdrive"
	MemorySize   = 128
	MaxDuration  = 60
	CodePath     = ".bin/"
...
func NewLambdaCdkStack(scope constructs.Construct, id string, props *LambdaCdkStackProps) awscdk.Stack {
	...
	testdriveFn := awslambda.NewFunction(stack, jsii.String(FunctionName), &awslambda.FunctionProps{
	...
	rule := awsevents.NewRule(stack, jsii.String("TestDriveScheduledRule"), &awsevents.RuleProps{
	...
	testdriveFn.AddPermission(jsii.String("AllowEventRule"),
	...
	rule.AddTarget(awseventstargets.NewLambdaFunction(testdriveFn, &awseventstargets.LambdaFunctionProps{}))
